...
const ClientProjectDetailPage = () => {
  ...
  const fetchBids = async () => {
    ...
    try {
      // Try using RPC function first (requires migration to be deployed)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data: rpcData, error: rpcError } = await (supabase.rpc as any)(
        'get_project_bids_with_profiles', 
        { _project_id: id }
      ) as { data: RpcBidResponse[] | null; error: Error | null };

      if (!rpcError && rpcData && Array.isArray(rpcData)) {
        console.log("RPC bids data:", rpcData);
        const transformedBids = rpcData.map((bid) => ({
          id: bid.id,
          ...
          freelancer: {
            first_name: bid.freelancer_first_name,
            last_name: bid.freelancer_last_name,
            profile_picture_url: bid.freelancer_profile_picture_url
          }
        }));
      ...
      // Fallback: fetch bids without profiles
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2.5 mb-2">
                            ...
                            <div className="text-right shrink-0">
                              ...
                              {bid.status !== 'pending' && (
                                <span className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded inline-block ${
                          ...
                          <p className={`text-[11px] text-slate-500 leading-relaxed mb-2 ${
                            expandedBidId === bid.id ? '' : 'line-clamp-1'
                          }`}>
                            {bid.proposal}
                          </p>
